	...
	(loop-for-count (?i 1 (length$ ?alimentos)) do
		...
		(if (neq ?cant [nil]) then
			...
			(if (neq ?macro [nil]) then
				(bind ?carbohidratos (+ ?carbohidratos (max 0.0 (send ?macro get-carbohidratos))))
				(bind ?fibra_alimentaria (+ ?fibra_alimentaria (max 0.0 (send ?macro get-fibra_alimentaria))))
				(bind ?proteinas (+ ?proteinas (max 0.0 (send ?macro get-numero_proteinas))))
			)
...
;	)))))))
	...
	(if (or (> (fact-slot-value ?cmenu proteinas) (fact-slot-value ?cdrmax proteinas)) (< (fact-slot-value ?cmenu proteinas) (fact-slot-value ?cdrmin proteinas))) then
			(return FALSE)
	)
	(return TRUE)
)
...
(defrule actualizar-puntuaciones-ingrediente-principal ""
	...
	(progn$ (?f ?fl)
		(modify ?f (puntuacion (+ (fact-slot-value ?f puntuacion) 5)))
	)
)
