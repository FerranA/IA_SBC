...
(deffunction suavizar-restricciones "" ()
	...
	(bind ?simbolo (nth$ 1 (find-fact ((?f cdr-maxima)) TRUE)))
	(bind ?simbolo (fact-slot-value ?simbolo nombre))
	(bind ?simbolo (symbol-to-instance-name ?simbolo))
	(bind ?macromax (send ?simbolo get-numero_macronutrientes))
	(bind ?micromax (send ?simbolo get-numero_micronutrientes))
	(bind ?minermax (send ?micromax get-numero_minerales))
	(bind ?vitammax (send ?micromax get-numero_vitaminas))
	(bind ?inc 1.1)
	(bind ?dec 0.9)
	(modify ?fact (a            	 (min (* (fact-slot-value ?fact a)                 ?inc) (send ?vitammax get-a)))
				 ;(acido_folico      (min (* (fact-slot-value ?fact acido_folico)      ?inc) (send ?vitammax get-acido_folico)))
				 ;(b_1               (min (* (fact-slot-value ?fact b_1)               ?inc) (send ?vitammax get-b_1)))
				 ;(b_2               (min (* (fact-slot-value ?fact b_2)               ?inc) (send ?vitammax get-b_2)))
			 	 ;(b_6               (min (* (fact-slot-value ?fact b_6)               ?inc) (send ?vitammax get-b_6)))
				 ;(b_12              (min (* (fact-slot-value ?fact b_12)              ?inc) (send ?vitammax get-b_12)))
				  (c                 (min (* (fact-slot-value ?fact c)                 ?inc) (send ?vitammax get-c)))
				  (d                 (min (* (fact-slot-value ?fact d)                 ?inc) (send ?vitammax get-d)))
				  (e                 (min (* (fact-slot-value ?fact e)                 ?inc) (send ?vitammax get-e)))
				 ;(k                 (min (* (fact-slot-value ?fact k)                 ?inc) (send ?vitammax get-k)))
				 ;(niacina           (min (* (fact-slot-value ?fact niacina)           ?inc) (send ?vitammax get-niacina)))
				  (calcio            (min (* (fact-slot-value ?fact calcio)            ?inc) (send ?minermax get-calcio)))
				 ;(cobre             (min (* (fact-slot-value ?fact cobre)             ?inc) (send ?minermax get-cobre)))
				 ;(fluoruro          (min (* (fact-slot-value ?fact fluoruro)          ?inc) (send ?minermax get-fluoruro)))
				 ;(fosforo           (min (* (fact-slot-value ?fact fosforo)           ?inc) (send ?minermax get-fosforo)))
				  (hierro            (min (* (fact-slot-value ?fact hierro)            ?inc) (send ?minermax get-hierro)))
				 ;(iodo              (min (* (fact-slot-value ?fact iodo)              ?inc) (send ?minermax get-iodo)))
				 ;(magnesio          (min (* (fact-slot-value ?fact magnesio)          ?inc) (send ?minermax get-magnesio)))
				  (potasio           (min (* (fact-slot-value ?fact potasio)           ?inc) (send ?minermax get-potasio)))
				 ;(selenio           (min (* (fact-slot-value ?fact selenio)           ?inc) (send ?minermax get-selenio)))
				  (sodio             (min (* (fact-slot-value ?fact sodio)             ?inc) (send ?minermax get-sodio)))
				 ;(zinc              (min (* (fact-slot-value ?fact zinc)              ?inc) (send ?minermax get-zinc)))
